...
import { fileSystems } from '../../src/loaders/FileLoader.js';

class EditorFileSystem {
	load( url, loader ) {
		const blob = window.fsAPI.readFile( url ).then(blob=> {return new Response( blob ); });
	}
}

fileSystems.unshift( new EditorFileSystem() );
...
function Loader( editor ) {
	...
	this.loadFromPaths = function ( paths ) {
		if ( paths.length > 0 ) {
			...
			for ( let i = 0; i < paths.length; i ++ ) {
				...
				scope.loadFromPath( paths[ i ], manager );
	...
	this.loadFromPath = async function ( path, manager ) {
		...
		switch ( extension ) {
...

			{
				...
				const loader = await createGLTFLoader();
				const promise = loader.load(path, function ( result ) {
